...
export const simulateRace = async (raceId) => {
    ...
    const interval = setInterval(async () => {
        try {
            ...
            // Recalculate points live
            await calculateRacePoints(Number(raceId));
...
export const stopSimulation = async (raceId) => {
    ...
    try {
        ...
        // set race back to SCHEDULED (it was not completed)
        await prisma.race.update({ where: { id: Number(raceId) }, data: { status: 'SCHEDULED' } });
